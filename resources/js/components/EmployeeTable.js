...
export default class EmployeeTable extends Component {
  ...
  loadAccount(){
    axios.get('http://127.0.0.1:8000/api/employeeaccount').then((response)=> {
      this.setState({
  ...
  editAccount(){
    ...
    axios.put('http://127.0.0.1:8000/api/employeeaccount/update/'+id, {name, email}).then((response)=> {
      this.setState({
        editAccountModal:!this.state.editAccountModal,
        data:{id:"", name:"", email:""}
      })
  ...
  deleteAccount(id){
    axios.put('http://127.0.0.1:8000/api/employeeaccount/delete/'+id).then((response)=> {
      this.loadAccount()
    })
  }
...
if (document.getElementById('EmployeeTable')){
    reactDom.render(<EmployeeTable />, document.getElementById('EmployeeTable'));
}