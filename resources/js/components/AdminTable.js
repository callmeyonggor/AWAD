...
export default class AdminTable extends Component {
  ...
  loadAccount(){
    axios.get('http://127.0.0.1:8000/api/adminaccount').then((response)=> {
      this.setState({
  ...
  editAccount(){
    ...
    axios.put('http://127.0.0.1:8000/api/adminaccount/update/'+id, {name, email}).then((response)=> {
      this.setState({
        editAccountModal:!this.state.editAccountModal,
        data:{id:"", name:"", email:""}
      })
  ...
  deleteAccount(id){
    axios.put('http://127.0.0.1:8000/api/adminaccount/delete/'+id).then((response)=> {
      this.loadAccount()
    })
  }
...
if (document.getElementById('AdminTable')){
    reactDom.render(<AdminTable />, document.getElementById('AdminTable'));
}